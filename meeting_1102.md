...
## **Original goals**
- 可以用 ```CARRADA Dataset``` 的資料讓 ```YOLO``` 跑起來
- 可以解決 ```RD Map``` 的 "多目標 多類別" 偵測問題
- Sub-goal
  - pipeline
...
## **Dataset preparation**
- Extract all rd_matrixs along with its annotations
![](https://i.imgur.com/nVQzWe0.png)
- The number of directorie: ```30```, the total number of labels / images: ```7193```
![](https://i.imgur.com/CUGhlpx.png)



## **Issues**
- Cannot train due to ```RuntimeError: tensor size mismatch```
- The bbox annotation seems **NOT** COCO format
- For instance
  - get a random sample and print out the index through ```__getitem__(self, index)```
![](https://i.imgur.com/a3T6OpL.png)
  - ```000114.png``` (index starts from ```35``` to ```177```, so ```80-th``` means ```114.png``` (the ```train.csv``` is 1-based indexing)
<img src='https://i.imgur.com/5O0lrBC.jpg' width=50% height=60%>
  - Plot the RDM with its bboxes
<img src='https://i.imgur.com/iZXeKb0.png' width=35% height=50%>
  - COCO format? 
![](https://i.imgur.com/zyMI4Rr.png)
  - YOLO format, ```000114.txt```
![](https://i.imgur.com/RKcTwZx.png)
  - After taking a closer look, this seems to be ```Pascal_VOC format```


## **Conclusion**
- Correct the foramt mistake
- Run any newer version of YOLO, e.g. YOLOX
  - Ge, Zheng, et al. "[Yolox: Exceeding yolo series in 2021](https://arxiv.org/abs/2107.08430)." arXiv preprint arXiv:2107.08430 (2021). 
    - 基於 YOLOv3 和 YOLOv5 基礎的 high-performance anchor-free YOLO
    - 在 Detection Head 的地方將 Coupled Head 換成 Decoupled Head，把 Class, Bounding Box, Objectness 切成不同分支來訓練
  - Ge, Zheng, et al. "[Ota: Optimal transport assignment for object detection](https://openaccess.thecvf.com/content/CVPR2021/papers/Ge_OTA_Optimal_Transport_Assignment_for_Object_Detection_CVPR_2021_paper.pdf)." Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition. 2021.



## **Appendix**
- [samples](https://github.com/paulchen2713/samples)
- [YOLO_project](https://github.com/paulchen2713/YOLO_project)
- ```convert_label.py```
```python=
# -*- coding: utf-8 -*-
"""
...
def main():
    ...
    for dir_name in dir_names:
        ...
        for key in all_keys:
            ...
            with open(RDM_PATH + f"RD2_log.txt", "a") as label_txt_file:
...
Creates a Pytorch dataset to load the Pascal VOC datasets with YOLO format annotation
"""
